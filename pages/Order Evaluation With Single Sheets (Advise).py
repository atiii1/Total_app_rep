...
if authentication_status:
    ...
    st.title("Order Evaluation with single sheets (Advise)")
    ...
    if uploaded_files:
        ...
        #st.session_state.download_completed_2 = False # clear flag of downloaded data
           ...
           # Radio button for processing type
            ...
            if process_type == "Process with Step Number":
                ...
                if filtered_sheets:
                    ...
                    # Initialize session state for the download checkbox if not already set
                    # Unique identifier for the current page
                    # Unique identifier for the current page
                    page_name = "page_2"  # Replace with a unique identifier for each page
                ...
                if st.button('Show Graph'):
                    ...
                    if selected_column in filtered_df.columns and group_by_column in filtered_df.columns:
                        ...
                        if not cleaned_df.empty:
                            ...
                            st.error("No data is available for the selected column.")
                    else:
            ...
            if cycle_time_column:
                ...
                if st.button('Show Table of Selected Data'):
                    if 'grouped_data' in st.session_state and not st.session_state.grouped_data.empty:
                        ...
                        st.warning("No data available to display. Please generate the graph first.")

            else:
                st.warning("Cycle time is not selected.")
        else:
            st.warning('Combined df is empty.')
    else:
        st.warning('Files are not uploaded.')
